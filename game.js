...
const RICO_TRIVIA = {
  'rico_default.png': {
    title: '制服のリコ先輩',
    cards: [
      { tag: '私生活', text: 'この制服はカジノ支給品だけど、ボタンの裏に小さく♠マークが彫ってある。新人時代の自分への験担ぎで、夜勤明けにナイフで削ったらしい。「あの夜、これがなきゃ多分辞めてたわよ。ちっぽけな印だけど、私の最初の覚悟」と本人談。' },
      { tag: 'ポーカー豆知識', text: 'プリフロップでAAを引く確率は約0.45%（221回に1回）。一見強そうでも、ヘッズアップでKKに負ける確率は約18%、ランダムな2枚にすら15%負ける。「最強の手ほど、勝率の天井を意識して打つの。120%勝てる手なんて、この世にないから」。' },
      { tag: '戦術ノート', text: 'リコの基本方針は「ポジションが価値の半分」。ボタン（最終アクション）から打つレンジは、UTG（最初）の約3倍に広げていい。理由は単純で、相手の反応を全部見てから決められるから。情報の非対称性こそが、長期で勝つ理由。' },
      { tag: 'こぼれ話', text: '休憩室のロッカーは上から3段目がリコ専用。中身は予備のチョーカー、目薬、塩飴、そしてなぜか単行本のミステリー小説が3冊。「ディーラー業はね、頭が冷めない夜があるの。そういう日は塩飴と推理小説。これで戻る」。' },
    ],
  },
  'rico_pajama.png': {
    title: 'パジャマのリコ先輩',
    cards: [
      { tag: '私生活', text: '寝相は驚くほど静か。ただし枕は必ず3つ抱える。1つは頭、1つは膝の間、1つは胸。「囲まれてないと落ち着かないのよ。卓に座ってる時間が長い職業病かもね、四方が空いてると不安になる」。週1で枕カバーを全部洗うのもこだわり。' },
      { tag: 'ポーカー豆知識', text: '睡眠不足は判断力を確実に削る。ある研究では、5時間睡眠の翌日はブラフ頻度が無意識に18〜22%上がり、コールド・コールが増える。「眠い日は、ポットコントロールに徹するの。派手な仕掛けは寝てから考える、これ鉄則」。' },
      { tag: '戦術ノート', text: '疲労時のメンタル管理として、リコは「3分の砂時計」を使う。難しい局面では決断を3分遅らせ、その間に呼吸を整える。「即決って格好いいけど、9割は感情の即決なの。3分待てば理性が戻る。それで取り返せた額、もう数えられない」。' },
      { tag: 'こぼれ話', text: '朝はホットミルクに蜂蜜とシナモン少々。「甘いのって、勝った日のご褒美感あるじゃない？　負けた日の慰めにもなる。万能なのよ、結局」。冬場はジンジャーを足す。彼女いわく「指先が冷えてるとカードを配るときに音が変わる」らしい。' },
    ],
  },
  'rico_bunny.png': {
    title: 'バニーのリコ先輩',
    cards: [
      { tag: '私生活', text: 'バニーの耳は3種類を使い分けている。普段はスパンコール、VIP対応の日はベルベット、本気で勝負を見届ける夜はサテン。「素材で気合いが変わるの。サテンの日は、客もなぜか緊張するのよ。気のせいじゃない、絶対」。耳のワイヤーは自分で曲げ直す。' },
      { tag: 'ポーカー豆知識', text: 'ディーラーのシャッフル基準は「リフル2回・ストリップ1回・カット1回」が業界標準。これを15秒以内にこなすのがプロ。リコは13秒台。「これ以上速くしようとすると、手首がつる。健康管理も技術のうちよ」。' },
      { tag: '戦術ノート', text: 'ディーラー視点での「席運」の話。実はテーブルの席順で、ポット獲得率に微妙な偏りが出る。理由は人間心理で、ディーラーの真正面はベットが大きくなりやすい（注目される錯覚）。「自分が客なら、ディーラーの斜め45度を選ぶわね。観察できる、でも目立たない」。' },
      { tag: 'こぼれ話', text: 'チップの音だけで額を当てられる。$1チップは軽く澄んだ音、$25は重く低い、$100は鈍くて短い。「目を閉じてても誤差5枚以内。10年やってると指じゃなく耳が覚えるのよ」。新人ディーラーへの伝統芸として披露されるが、誰もまだ追いついていない。' },
    ],
  },
  'rico_casual.png': {
    title: '私服のリコ先輩',
    cards: [
      { tag: '私生活', text: '休日は古本屋を3軒はしごする。ポーカー戦略本はもう棚に並べきれないので、買うのは心理学・ミステリー・行動経済学ばかり。「フィクションの嘘の付き方を勉強すると、相手の嘘も見えるようになるの。物語ってね、ブラフの構造そのものよ」。' },
      { tag: 'ポーカー豆知識', text: 'プレイヤーの約85%は「自分は平均以上に上手い」と信じている（ダニング＝クルーガー効果）。算数的にあり得ない。リコ曰く「自分を疑える人だけが伸びる。私もね、いまだに毎日ハンド復習してるわよ。当たり前」。' },
      { tag: '戦術ノート', text: 'リコの「カフェ観察術」は実はバトルでも使える。喫茶店で隣の客の注文を予測する遊び（コーヒー or 紅茶、甘いもの有無、滞在時間）。これを毎週やっていると、ポーカー卓での「次のアクション予測」が驚くほど鋭くなる。「人間のパターンって、思ったより少ないの」。' },
      { tag: 'こぼれ話', text: 'お気に入りのカフェは3軒。マスターと顔なじみで、リコが入ると黙ってアールグレイが出てくる。「会話なしの常連、これ最高でしょ？　仕事で喋りすぎるから、休みの日は静かにしたいの」。窓側の角席が定位置で、客の入りを観察するのが趣味。完全に職業病。' },
    ],
  },
  'rico_dress.png': {
    title: 'ドレスのリコ先輩',
    cards: [
      { tag: '私生活', text: 'このドレスは初給料で買った勝負服。値段は半月分の家賃と同じ。「人生で一番震えた買い物。でもね、これ着てから負けた覚えがないのよ、不思議と。ジンクスって、自分で作ると本当に効く」。クリーニングは年2回だけ、本気の夜の前後にだけ出す。' },
      { tag: 'ポーカー豆知識', text: 'VIPルームのレートは表の卓の10倍以上。$25の代わりに$250、$100の代わりに$1000。けれど勝つために必要な技術は驚くほど同じ。「金額で技術が上がるわけじゃないの。怖がる方が、負ける」。ハイレートで強いのはメンタルが鈍い人、ではなく感情を分離できる人。' },
      { tag: '戦術ノート', text: 'VIP卓の暗黙ルール：1ハンドあたりの平均アクション数は意外に少ない（プリフロップで決着が3割、フロップ早終わりが2割）。理由はみんなレンジが狭く、強い手で殴り合うから。「ブラフが効く場面が限られるの。だからこそ、その瞬間の見極めが全て」。' },
      { tag: 'こぼれ話', text: 'ヒールは必ず7cm。「これ以上だと、退場時の歩き方が雑になるの。勝った夜も負けた夜も、入退場は美しく。これが私の流儀」。実はヒールの中に小さなお守り（祖母にもらった硬貨）を仕込んでいる。誰にも見せたことはない。' },
    ],
  },
  'rico_kimono.png': {
    title: '和装のリコ先輩',
    cards: [
      { tag: '私生活', text: '帯は自分で結ぶ派。「人に締めてもらうと、間合いがズレるのよ。1cmの締まり具合で、立ち居振る舞い全部変わるから」。母から譲り受けた帯留めが3つあり、勝負の夜ごとに使い分ける。一番大事なのは翡翠で、これは年に1〜2回しか出さない。' },
      { tag: 'ポーカー豆知識', text: '日本にポーカーが伝来したのは明治初期、横浜の外国人居留地から。当初は花札と混ざった独自ルールが各地で生まれ、東京と大阪で役の優劣が違ったらしい。標準化されたのは戦後、進駐軍がアメリカ式を持ち込んでから。「文化って、混ざりながら育つのね」。' },
      { tag: '戦術ノート', text: '和装で打つ夜のリコは、不思議とブラフが通りやすいとジンクスがある。「相手が私の動作を読みづらいんだと思う。袖が動きを隠してくれるの。和装は最強のポーカーフェイスかもね、本当の意味で」。冗談に聞こえるが、実際の勝率データは出していない。' },
      { tag: 'こぼれ話', text: '抹茶を点てるとき、茶筅を振る音で集中スイッチが入る。「シャカシャカ、って音、心拍を整えるのに最高なの」。卓に着く前のルーティンとして、控室で必ず一服する。茶碗は祖母譲りで、欠けた縁を金継ぎで直したもの。「傷を隠さないの、これがいいのよ」。' },
    ],
  },
  'rico_swimsuit.png': {
    title: '水着のリコ先輩',
    cards: [
      { tag: '私生活', text: '泳ぎは平泳ぎ専門。「クロールは派手すぎてバレるじゃない、何かが」。何がバレるのかは本人も説明しない。プールサイドではいつも本を読んでいて、水に入っているのは全体の1割程度。「日光浴も大事よ、ビタミンD」。' },
      { tag: 'ポーカー豆知識', text: '夏場のカジノは空調がきつく、湿度が30%を切ることが多い。手汗が出にくくなる代わりに、唇や指先が乾く。「手汗で読まれる季節は冬。夏は逆に、無感情に見えすぎてブラフが通らない。季節で戦略変えるの、当たり前よ」。' },
      { tag: '戦術ノート', text: 'リゾートカジノでの観察談：休暇中の客は普段より「リスク許容度が2割上がる」傾向がある。理由は「非日常モード」で財布の感覚が緩むから。リコの提案：「リゾートでは相手のオールインを軽く扱わない。本気で来てる確率、街中の卓より高いから」。' },
      { tag: 'こぼれ話', text: 'ビーチでも砂にチップ模様を描いて練習する。完全に職業病。「友達に呆れられるけど、これが落ち着くのよ。あと、波の音ってBGMとして最高。脳の使ってない部分が休まる感じ」。日焼け止めはSPF50を完璧に塗る、手の甲は二度塗り。理由：「商売道具だから」。' },
    ],
  },
  'rico_gym.png': {
    title: 'ジム服のリコ先輩',
    cards: [
      { tag: '私生活', text: '週3でジム通い、もう4年続いている。スクワット・デッドリフト・懸垂が三本柱。「上半身だけ鍛える人多いけど、下半身が安定すると、表情も安定するの。卓に座ってる時間が長い職業なんだから、お尻と背中こそ大事」。コーチには「ポーカーディーラーで一番強い人」と認定されている。' },
      { tag: 'ポーカー豆知識', text: '長時間プレイは判断力を確実に下げる。研究によると、3時間以上連続でプレイしたプレイヤーは「明らかな-EV判断」を1時間あたり2〜3回多くする。プロは「セッション90分・休憩15分」を厳守する人が多い。「集中力って、有限の体力なのよ。筋力と同じ」。' },
      { tag: '戦術ノート', text: 'リコ流のメンタルリセット：トイレ休憩で必ず10秒だけ思いっきり背伸びをする。「血流が変わると、頭の中の霧が晴れるの。脳に酸素入れるイメージ。マインドフルネスとか難しいこと言わないわよ、ただ伸びるだけでいい」。長期戦の生命線。' },
      { tag: 'こぼれ話', text: 'プロテインはバニラ味一択。「チョコは1週間で飽きる、ストロベリーは甘すぎる、抹茶は本物の方が美味しい。バニラは無難で長く付き合える、結婚相手と同じ」。トレーニング後の儀式として、必ずジムの自販機でブラックコーヒーを買って飲む。' },
    ],
  },
  'rico_school.png': {
    title: '学生風のリコ先輩',
    cards: [
      { tag: '私生活', text: '昔は数学だけが得意だった。国語は赤点ギリギリ、英語は壊滅的、でも数学は校内で表彰されたこともある。「確率の問題、解いてる時が一番楽しかったの。今思えば、あの時からポーカーへの道は決まってたのね、たぶん」。教科書は今でも実家の本棚にある。' },
      { tag: 'ポーカー豆知識', text: 'ポットオッズの計算式は単純：必要コール額 ÷ コール後のポット総額 = 必要勝率。例：$50コールでポットが$200になるなら、25%以上勝てれば+EV。慣れれば3秒で出る。「九九と同じよ。最初は紙に書いて、そのうち口が覚える」。' },
      { tag: '戦術ノート', text: 'リコの中学時代の数学ノートに、すでに「期待値」という概念が書き込まれていた。「ダイスを振って、何回やったら平均いくらになる？」みたいな問題が大好きだった。「結局ポーカーって、感情を排した期待値ゲームなの。算数の延長なのよ、本当は」。' },
      { tag: 'こぼれ話', text: '放課後の屋上で初めてカードを覚えた。教えてくれたのは……「内緒」。誰に聞いても口を割らない。「ある人。今でも年に一度だけ、卓を挟むの。私が勝った日はその人がコーヒーを奢って、負けた日は私が奢る。それだけのルール、もう10年続いてる」。' },
    ],
  },
  'rico_witch.png': {
    title: '魔女のリコ先輩',
    cards: [
      { tag: '私生活', text: 'ハロウィンは年に一度のお楽しみ。「コスプレ警察、私はしないわよ。みんな好きにすればいいの。むしろ普段着てない服を着ると、別人格が出てきて楽しいじゃない？」。仮装の準備は1ヶ月前から始め、毎年テーマを変える。今年は「闇のカジノ女将」。' },
      { tag: 'ポーカー豆知識', text: '心理的アンカリングという現象がある。最初のベット額が、その後の判断基準になってしまう人間の癖。「最初に$10をベットしたら、$50がやけに大きく見えるの。これを利用して、序盤に小さく入れて中盤で爆発させる戦術もある。アンカーを操作する側に回るのよ」。' },
      { tag: '戦術ノート', text: '「呪いをかけた」と宣言してから打つと、相手が3%多くフォールドするという都市伝説。リコの個人記録では「ハロウィン期間限定で実際に5%上がった」とのデータがあるらしいが、サンプル数が少ないので統計的有意性は不明。「でも気分は良いの、それが大事」。' },
      { tag: 'こぼれ話', text: '本物の黒猫を飼っている。名前は「フロップ」。元野良で、雨の日にカジノの裏口で鳴いていたのを連れ帰った。「最初の3日間、ベッドの下から出てこなかったの。でもね、ある朝起きたら枕元にいて、それからずっと一緒。家族って、こうやって増えるのね」。' },
    ],
  },
  'rico_santa.png': {
    title: 'サンタのリコ先輩',
    cards: [
      { tag: '私生活', text: 'クリスマスは仕事納め後、一人でケーキを食べるのが恒例。「これがいいの、贅沢でしょ？　誰にも気を遣わず、好きなだけ生クリームを乗せられる。年に一度くらい、こういう日があってもいいじゃない」。ケーキは毎年同じ店のショートケーキで、3年連続予約している。' },
      { tag: 'ポーカー豆知識', text: '年末のカジノは寄付付きトーナメントが多い。バイインの10〜20%が慈善団体に行く形式で、世界中で開催される。「負けても気分はあったかいのよ、税控除にもなるし。私は毎年1つは出るって決めてる、これは自分への約束」。' },
      { tag: '戦術ノート', text: 'トーナメント終盤の「バブル」戦術論：賞金圏ギリギリでは、短いスタックが極端にタイトになる。これを逆手に取り、中堅スタックが小スタックを狙って積極的にプレッシャーをかけるのが定石。「人の弱気を、自分の利益に変えるの。冷たいって思う？　ゲームよ、これは」。' },
      { tag: 'こぼれ話', text: 'プレゼント包装が異常に上手い。リボンの角度、テープの位置、すべて完璧。「チップを並べるのと同じ手つきなのよ、たぶん。整然としたものが好きなの、私」。職場ではクリスマス前になると、同僚から「ラッピング代行」の依頼が殺到する。報酬はお菓子。' },
    ],
  },
};
